                    ...
                    case "mv":
                        switch (ope.selected.type) {
                            ...
                            case "ellipse":
                                ...
                                ope.showTracker(ope.selected.getBBox());
                                break;
                            case "path": //also polygons
                                var xdiff = ope.selected.getBBox().x - (event.clientX - ope.onHitXY[0]);
                                var ydiff = ope.selected.getBBox().y - (event.clientY - ope.onHitXY[1]);
                                for (var i = 0; i < ope.selected.path.length; i++) {
                                    if (ope.selected.path[i].type == "line" || ope.selected.path[i].type == "move") {
                                        ope.selected.path[i].arg[0] = ope.selected.path[i].arg[0] - xdiff;
                                        ope.selected.path[i].arg[1] = ope.selected.path[i].arg[1] - ydiff;
                                    }
                                }
                                ope.selected.redraw();
                                ope.showTracker(ope.selected.getBBox())
...
VectorEditor.prototype.serialize = function(){
    var shapearray = [];
...
VectorEditor.prototype.onHit = function(event){
    switch (ope.mode) {
        case "select":
            ope.mouseaction = "mv";
...
VectorEditor.prototype.hideTracker = function(){
    if (ope.tracker) {
...
VectorEditor.prototype.showTracker = function(bounds){
    //settings
